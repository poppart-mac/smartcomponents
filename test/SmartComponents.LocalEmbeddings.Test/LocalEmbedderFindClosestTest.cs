...
public class LocalEmbedderFindClosestTest
{
    ...
    public void CanFindClosestMatches_Static()
    {
        ...
        var closestWithScore = LocalEmbedder.FindClosestWithScore(
            embedder.Embed("beans"),
            embeddedCandidates,
            2
        );
        ...
        Assert.Collection(
            closestWithScore.Take(2),
            result =>
            {
                Assert.Equal("Coffee", result.Item);
                Assert.InRange(result.Similarity, 0, 1.01f);
            },
            result =>
            {
                Assert.Equal("Latte", result.Item);
                Assert.InRange(result.Similarity, 0, 1.01f);
            }
        );
    }
    ...
    public void CanFindClosestMatches_Instance()
    {
        ...
        var closest = embedder.FindClosest(
            new() { SearchText = "beans", MaxResults = 2 },
            embeddedCandidates
        );
        var closestWithScore = embedder.FindClosestWithScore(
            new() { SearchText = "beans", MaxResults = 2 },
            embeddedCandidates
        );
    ...
    public void CanSpecifySimilarityThreshold_Static()
    {
        ...
        Assert.Collection(
            ...
            {
                Assert.Equal("Coffee", result.Item);
                Assert.InRange(result.Similarity, 0.95f, 1.01f);
            }
        );
    }
}
