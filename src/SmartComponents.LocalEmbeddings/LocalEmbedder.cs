...
public sealed partial class LocalEmbedder(string modelName, BertOnnxOptions options)
    : IDisposable,
        ITextEmbeddingGenerationService
{
    ...
    public LocalEmbedder(
        ...
        : this(
            modelName,
            new BertOnnxOptions { CaseSensitive = caseSensitive, MaximumTokens = maximumTokens }
        ) { }
    ...
    public EmbeddingF32 Embed(string inputText) => Embed<EmbeddingF32>(inputText, null);
    ...
    public async Task<TEmbedding> EmbedAsync<TEmbedding>(
        string inputText,
        Memory<byte>? outputBuffer = default
    )
        where TEmbedding : IEmbedding<TEmbedding>
    {
        var embedding = (await _embeddingGenerator.GenerateEmbeddingsAsync([inputText])).Single();
        return TEmbedding.FromModelOutput(
            embedding.Span,
            outputBuffer ?? new byte[TEmbedding.GetBufferByteLength(embedding.Span.Length)]
        );
    }
    ...
    public IList<(string Item, EmbeddingF32 Embedding)> EmbedRange(IEnumerable<string> items) =>
    ...
    public void Dispose() => _embeddingGenerator.Dispose();
